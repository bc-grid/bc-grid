...
function encodeColumnFilterInput(filter: ServerColumnFilter): string | undefined {
  ...
  // The default `contains` filter with no modifiers serialises as a plain
  // string for legacy compatibility — pre-extend persistence payloads
  // round-trip unchanged. Anything non-default emits JSON so the editor
  // restores operator + modifier flags faithfully.
 ...
 * to compile drops the filter (no match) — a hand-built filter with a
 * malformed pattern shouldn't blow up the predicate.
 */
 ...
 * filter inputs typed under the pre-extend code path — and persisted
 * payloads that pre-date this rescue — still resolve. Default op is
 * `contains` per `filter-registry-rfc §text`.
 */
 ...
 * minimal and so persistence round-trips stay tight.
 */
