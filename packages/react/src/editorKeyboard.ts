...
export function getEditorEditModeKeyboardIntent(
  ...
  // shortcuts, Ctrl+Enter is form-submit on many hosts. Letting any
  // of them flow through to the wrapper would steal a host shortcut
  // and silently drop a typed commit at the same time.
  if (hasNonShiftModifier(input)) return { type: "ignore" }
  if (input.key === "Enter") {
