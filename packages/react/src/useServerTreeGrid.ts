 ...
 * ```
 */
export type UseServerTreeGridServerProps<TRow> = Omit<BcServerTreeProps<TRow>, "columns">

export interface UseServerTreeGridResult<TRow> {
  props: UseServerTreeGridServerProps<TRow>
...
export function useServerTreeGrid<TRow>(
  ...
  const props = useMemo<UseServerTreeGridServerProps<TRow>>(
    () => ({
      apiRef,
      gridId,
      rowId,
