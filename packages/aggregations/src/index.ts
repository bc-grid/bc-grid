...
export function aggregate<TRow, TValue, TResult, TAcc>(
  ...
  const ctx = createContext(column, options)
...
export function aggregateColumns<TRow>(
  ...
  return columns.flatMap((column) => {
    const aggregation = resolveAggregation(column.aggregation)
...
function resolveAggregation(definition: BcAggregation | undefined): Aggregation | null {
...
function createContext<TRow, TValue>(
  column: BcGridColumn<TRow, TValue>,
  options: AggregateOptions,
...
function getColumnValue<TRow, TValue>(row: TRow, column: BcGridColumn<TRow, TValue>): TValue {
