 ...
 * migrations (`select`, `multi-select`; follow-up: `autocomplete`).
 *
 ...
 *     every toggle. Used by `multiSelectEditor`. The listbox stays open
 *     until the editor portal commits via Tab/Enter/Escape.
 *
 ...
 *   - Type-ahead by single-key prefix match. In multi-mode the key
 *     navigates the active option without toggling — Space toggles.
 ...
 * `readonly TValue[]` (multi) is stashed on the trigger element via a
 * stable JS property key, so the editor portal's `readEditorInputValue`
 * can return the typed value on click-outside / Tab without going
 * through `column.valueParser`. Mirrors the existing
 * `__bcGridSelectOptionValues` contract on native `<select>`.
 */
...
export function Combobox(props: ComboboxProps): ReactNode {
  ...
  const initialMultiValues = isMulti
    ? toReadonlyArray(props.initialValue as readonly unknown[])
    : null
  const initialIndices = isMulti
    ? selectedIndicesFromValues(options, initialMultiValues ?? [])
    : findOptionIndexByValue(options, props.initialValue as unknown)
  const seedIndex = findOptionIndexBySeed(options, seedKey)

  // selectedIndices is a Set in multi mode; in single mode it's
  // either an empty Set or a singleton derived from `initialIndices`.
  const [selectedIndices, setSelectedIndices] = useState<ReadonlySet<number>>(() => {
    if (isMulti) return new Set(initialIndices as number[])
  ...
  const [activeIndex, setActiveIndex] = useState(() => {
    ...
    if (isMulti) {
      const first = (initialIndices as number[])[0]
      return first ?? 0
    }
  ...
  // Re-runs whenever the user picks a new value.
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>) => {
      ...
      if (event.key === "Enter") {
        ...
        // Enter also bubbles up so the wrapper commits the current
        // selection array.
      ...
      // Single mode: also auto-selects (mirrors native `<select>`).
      // Multi mode: just navigates — Space toggles.
      if (
        event.key.length === 1 &&
        !event.ctrlKey &&
        !event.metaKey &&
  ...
  const selectedSummary = describeSelectedSummary(options, selectedIndices, isMulti)
  const triggerSwatch = selectedSummary.singleSwatch
      ...
      >
        ...
        {isMulti ? (
          <span className="bc-grid-editor-combobox-chips" data-bc-grid-editor-combobox-chips="true">
            {selectedSummary.chips.length === 0 ? (
          ...
          <span className="bc-grid-editor-combobox-label">{selectedSummary.label}</span>
        )}
