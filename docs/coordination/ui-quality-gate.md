...
- **REJECT:** Icon buttons whose icon is text glyph (`x`, `>`, `…`) instead of an inline SVG. Text glyphs don't adapt across light / dark / forced-colors and don't accept stroke-width tuning. Use the shared lucide-backed icon modules (`internal/header-icons.tsx`, `internal/pagination-icons.tsx`, `internal/panel-icons.tsx`, `internal/disclosure-icon.tsx`) or add a new lucide-backed module if a new family is genuinely missing.
