...
> **Alpha.3 / GA prerequisites (gathering):**
> - `v05-server-mode-switch` stage 3.3 — RFC §9 test sweep (14 carry-over unit cases + 1 Playwright happy-path). worker1, ~3-4h.
> - `v05-use-server-grid-polymorphic-hook` — alpha.3 / GA per the mode-switch RFC §10 Q6 split. worker1, ~6-8h.
> - `v06-layout-architecture-pass` — RFC at `docs/design/layout-architecture-pass-rfc.md` ratified 2026-05-03. Three-PR sequence (worker1 / worker2 / worker3), ~18-25h. **Recommendation:** ship layout-pass PR (a) before v0.5.0 GA so the bsncraft consumer's open layout memos (sticky-left detail, header scroll lag) close in v0.5 stable rather than waiting for v0.6.
> - `v06-in-cell-editor-mode` — RFC at `docs/design/in-cell-editor-mode-rfc.md` ratified 2026-05-03. Three-PR sequence (worker3), ~16-22h. v0.6 scope; doesn't block v0.5 GA but composes naturally with the layout pass.
