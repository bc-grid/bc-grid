...
> - **Server mode switch with state carry-over (committed v0.5.0 scope, 2026-05-03):** maintainer call: "this should have been day-1 functionality when server mode + grouping was supported." A grid driven by `<BcServerGrid>` (or its successor) must hold one set of shared state (filters, sort, selection, focus, scroll) and switch its row-model engine internally when `groupBy` flips between `[]` and non-empty, instead of forcing consumers to mount one component per mode and lose carry-over on the switch. RFC at `docs/design/server-mode-switch-rfc.md` (**delivered 2026-05-03, awaiting maintainer ratification of 8 open questions in §10**). Recommendation: Shape A (`<BcServerGrid>` becomes mode-polymorphic internally; controlled `groupBy` drives the switch); the three turnkey hooks remain as escape-hatches; new `useServerGrid` polymorphic hook lands in alpha.3 / GA. Worker1 owns implementation as the next lane after their context-menu work.
