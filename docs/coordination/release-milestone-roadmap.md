...
> **v0.6.0 plan (consumer-feedback absorption + spreadsheet flows train):**
> - **Bsncraft P0 #4 carry-over:** `v05-bsncraft-pinned-scroll-shadow-overlay` — pseudo-element gradient at pinned boundary paints over row hover bg. Worker2 next-up; likely fix `mix-blend-mode: multiply` or negative z-index on the pseudo.
> - **Bsncraft 16-issue feedback memo carry-over:** remaining P1/P2 items (5, 7, 8, 11, 12, 13, 14, 15, 16) — most fixed in main; the GA cut surfaces them to bsncraft.
> - **Worker1 v0.6 train:** `v06-stale-response-flood-test` (planning doc §9), `v06-server-tree-stale-viewkey-fetches` (§10), `v06-server-view-change-reset-policy` (§1).
> - **Worker2 v0.6 train:** `v06-saved-view-storage-recipe` (consumer-side persistence layer composing with #423 DTO), continuing planning doc §7+ (grouping + filter follow-ups).
> - **Worker3 v0.6 train:** `v06-prepareresult-preload-select-multi` (planning doc §3 — async-loaded options for select + multi-select), `v06-editor-tab-wraparound-polish` (followup to #431).
> - **Original v0.6 plan (Range, Clipboard, Spreadsheet Flows):** preserved below; train alongside the consumer-feedback absorption work.
