...
> - **Re-evaluate:** `useServerTreeGrid` dual-output (paged ⇄ tree mode-switch) — previously cancelled when bsncraft moved to `<BcServerGrid>`. The 2026-05-03 consumer review re-flags it as "the more important deliverable" before v0.5.0 stable. Consumer is still routing through their own discriminated wrapper because the hook trio doesn't expose mode-switching. Worth a fresh look — likely a v05-server-mode-switch task in the next planning pass.
