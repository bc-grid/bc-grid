...
> **Alpha released (2026-05-03):** tag `v0.5.0-alpha.1` published all 11 packages to GitHub Packages (release workflow run `25263638850`). All audit P0 items closed except the bsncraft customers migration (in-flight, bsncraft team drafting). All coordinator gates green at the alpha-1 cut: type-check, lint, 1262/0 unit tests, build, api-surface, bundle-size (91.12 KiB / 100 KiB hard cap), tarball-smoke, release-preflight, 142/142 Playwright e2e, 3/3 smoke-perf (1 deferred-skip). bsncraft can now consume `@bc-grid/*@0.5.0-alpha.1` from the registry; alpha.2 cuts after the queued items below land.

> **Alpha.2 prerequisites (gathering):**
> - `v05-row-class-name-and-style` (queued, coordinator-owned) — conditional row tinting requested for ERP master tables.
> - `bsncraft` v0.5 alpha.1 consumer paper-cuts (#65464d0 — fixed 2026-05-03): pinned-row hover too loud against saturated host accents (option A — inherit row-hover); tree validator throws now console.error in dev across paged/infinite/tree; `ServerTreeRow` re-exported from `@bc-grid/react`. Pre-conditions for v0.5.0 stable per consumer review.
> - `bsncraft` customers grid migration findings (in flight on bsncraft side; the consumer is on `<BcServerGrid rowModel="paged"|"tree">` now via a host-side discriminated wrapper, has not yet adopted the `useServerPagedGrid`/`useServerTreeGrid` turnkey hooks).
> - **Server mode switch with state carry-over (committed v0.5.0 scope, 2026-05-03):** maintainer call: "this should have been day-1 functionality when server mode + grouping was supported." A grid driven by `<BcServerGrid>` must hold one set of shared state (filters, sort, selection, focus, scroll) and switch its row-model engine internally when `groupBy` flips between `[]` and non-empty, instead of forcing consumers to mount one component per mode and lose carry-over on the switch. RFC at `docs/design/server-mode-switch-rfc.md` **ratified 2026-05-03**: Shape A confirmed; 7 of 8 open questions resolved with the coordinator recommendation, **Q1 lands as hybrid** (sync state setters per AG Grid SSRM, plus a Promise-returning `apiRef.whenIdle()` getter for consumers who want await semantics). New `useServerGrid` polymorphic hook split out to alpha.3 / GA per Q6. Worker1 owns implementation as the next lane after the context-menu server-toggles ship; estimate 18-25h, single PR.
