...
- `[in-flight: worker3]` **server-edit-sort-filter-contract-v040** — harden server-backed edit grid behavior when sort/filter view changes refetch pages while optimistic edits are pending. Branch: `agent/worker3/server-edit-sort-filter-contract-v040`. **Effort**: S.
