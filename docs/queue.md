...
- `[review: worker4 #277]` **sidebar-focus-audit** — audit pass over sidebar + tool panels (open / close, tab order, Escape, focus return, ARIA labels). One small safe fix: `BcFiltersToolPanel`'s "Clear all" button gets `aria-label="Clear all filters"` so AT users hear the full intent (button text "Clear all" alone is ambiguous). Coordination doc gains an audit table for every sidebar surface plus the next implementation tasks (clean column tool panel "Up" / "Down" labels, sidebar landmark naming, tool panel section headings). No focus traps, no runtime deps; lock-in markup tests for the new aria-label. **Effort**: S.
