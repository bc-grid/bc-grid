...
- `[in-flight: worker2]` **shadcn-menu-visual-polish-v030** — visual polish pass on the context menu + column-visibility menu chrome to match shadcn DropdownMenu conventions. Tighter rows + transition-colors, distinct keyboard-focus state via `--bc-grid-accent-soft`, full-bleed separators, `pointer-events: none` on disabled rows, and an opt-in `[data-variant="destructive"]` rule for custom destructive items (paired with a `variant?: "destructive"` field on `BcContextMenuCustomItem`). Tokens-only — no new direct shadcn-token reads. Companion to worker2's open #259 menu-item primitive (lands cleanly with or without it). Updates `docs/coordination/radix-shadcn-chrome-cleanup.md`. **Effort**: S.
