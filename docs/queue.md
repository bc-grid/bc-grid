...
- `[review: worker4 #185]` **server-tree-mode** — `rowModel="tree"`: lazy children fetching via `LoadServerTreeChildren`; expand/collapse triggers fetch; `ServerTreeRow` rendering. The implementation skeleton landed via integrator #112 (originally PR #90); this PR closes the reserved follow-up items called out in #90's description: treegrid ARIA polish per `accessibility-rfc §grid vs treegrid` (role="treegrid" + aria-level / aria-posinset / aria-setsize on tree-mode rows). **Branch**: `agent/worker4/server-tree-mode`. **Effort**: M.
