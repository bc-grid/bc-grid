...
- `[in-flight: worker3]` **server-edit-semantics-v040** — 0.4 planning pass for server-backed editable grids: focused tests/docs for `BcServerGrid` edit commit semantics, optimistic mutation queue behavior, rollback/error expectations, and consumer-owned reload/validation policy. **Branch**: `agent/worker3/server-edit-semantics-v040`. **Effort**: XS.
