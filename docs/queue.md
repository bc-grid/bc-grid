...
- `[in-flight: worker5]` **sidebar-tool-panel-shadcn-polish-v040** — audit sidebar/tool-panel chrome against the shadcn/Radix cleanup direction and land one safe CSS/token slice without changing column/filter/pivot panel behavior. **Branch**: `agent/worker5/sidebar-tool-panel-shadcn-polish-v040`. **Effort**: XS.
