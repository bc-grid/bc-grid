...
- `[review: worker3 #209]` **server-edit-grid-example-v020** — focused docs/example path for server-backed editable business grids: server sort/filter query shape, row identity, edit commit flow, optimistic vs reload behavior, and rollback expectations. **Branch**: `agent/worker3/server-edit-grid-example-v020`. **Effort**: S.
