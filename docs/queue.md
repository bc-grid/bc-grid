...
- `[review: worker2 #272]` **context-menu-docs-v040** — docs/examples polish for the context menu surface. Expands `docs/api.md §contextMenuItems` with a worked end-to-end recipe that composes the default items + filter + column + bulk built-ins + a custom row-action; documents the disabled-predicate (`(ctx) => boolean`) recipe with row context, the conditional-suppress-via-`false` recipe, and how `ctx.cell` / `ctx.column` / `ctx.row` differ across header / filter-row / data-cell triggers. Adds a new `apps/docs` recipe page mirroring `editor-custom-recipe.astro` / `custom-filters.astro` so the worked example lives on the docs site too. Pins the example shapes via a focused `contextMenuRecipes.test.ts` so the docs don't drift from the public types. **Effort**: S.
