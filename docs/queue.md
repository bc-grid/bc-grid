...
- `[in-flight: worker1]` **range-paste-apply-v040** — internal TSV paste apply helpers: parse diagnostics, anchor/range planning, editability/valueParser/validation checks, and atomic commit/patch plan output without UI clipboard wiring. **Branch**: `agent/worker1/range-paste-apply-v040`. **Effort**: S.
