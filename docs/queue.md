...
- `[review: worker2 #215]` **filter-operator-docs-and-api-v030** — pin the v0.3 text filter operator + modifier API in `docs/api.md` §4.4 and `docs/design/filter-registry-rfc.md`. Documents the wire-shape additions (`ServerColumnFilter.caseSensitive` / `regex`), the persistence shape (plain string for legacy default-contains, JSON for non-default), and the legacy compatibility note. Docs-only; coexists with #208 (filter-text-impl-extend implementation). **Effort**: XS.
