...
- `[in-flight: x1]` **api-surface-diff** — `tools/api-surface/` script that walks every package's built `dist/index.d.ts`, extracts the export names, and diffs against an expected manifest derived from `docs/api.md §9`. CI step in smoke job; fails the build on drift. Catches over-exports (helper functions leaking out of `@bc-grid/animations`) and under-exports (api.md promises a name the package doesn't ship). **Effort**: 1-2 days.
