...
- `[in-flight: worker1]` **visual-selection-layer** — v0.3 active range overlay polish: absolute-positioned range rectangles through virtualization and pinned columns; no fill handle. **Branch**: `agent/worker1/range-overlay-polish-v030`. **Effort**: M.
