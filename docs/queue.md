...
- `[in-flight: worker3]` **checkbox-editor-v040** — built-in boolean checkbox editor for 0.4 editing. Native checkbox input, Space toggles while editing, Enter / Tab / Escape remain grid-owned, optional tri-state only if it stays small and explicit. Branch: `agent/worker3/checkbox-editor-v040`. **Effort**: S.
