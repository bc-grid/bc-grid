...
- `[review: worker2 #295]` **header-focus-menu-state-v040** — finish the header chrome polish around hover / focus / open states. Tightens the resize-handle `::before` / `::after` selectors so the affordance only reacts when the resize handle itself is hovered or actively dragged (`data-bc-grid-resizing="true"`); removes the cell-level `:hover` and `:focus-within` activations that previously fired the neighbouring column's separator whenever the menu / filter button was focused or the header label was hovered. Adds a Radix-DropdownMenuTrigger-style `[data-state="open"]` rule on `.bc-grid-header-menu-button` mirroring the existing filter-button rule, and threads a `columnMenuOpen` flag through `renderHeaderCell` (sourced from a per-column `columnId` now stored alongside the column-menu anchor in `grid.tsx`) so only the trigger that opened the menu lights up — not every menu trigger in the row. Tokens-only; no behaviour rewrites. Markup tests pin `data-state` + `aria-expanded` on the menu trigger; CSS-contract tests pin the tightened resize-affordance scope and the menu-button open-state highlight. **Effort**: S.
