...
- `[in-flight: worker3]` **server-grid-request-semantics-v030** — harden server-backed request semantics for sort/filter/search/group/visible columns, pagination, refresh/invalidate sequencing, stale response ordering, and diagnostics snapshots. **Branch**: `agent/worker3/server-grid-request-semantics-v030`. **Effort**: S.
