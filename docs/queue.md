...
- `[in-flight: x2]` **infinite-mode-block-cache** — `rowModel="infinite"`: `ServerBlockCache` with LRU eviction; `LoadServerBlock` integration; viewport-driven block fetching with prefetch ahead. **Branch**: `agent/x2/infinite-mode-block-cache`. **Effort**: L (3-5 hours).
