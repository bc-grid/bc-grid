...
- `[in-flight: worker5]` **master-detail-production-polish-v030** — production polish for master/detail expansion states: instant row layout, no text scaling or row height morphing, safe detail-content reveal only, reduced-motion contract, and compact async child-state guidance. **Effort**: S.
