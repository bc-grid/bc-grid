...
- `[in-flight: worker1]` **fill-handle** — v0.2 rescue scope: visible active-range corner handle only. Drag-to-fill/copy behavior remains blocked until the paste/edit batch path is stable. **Branch**: `agent/worker1/fill-handle-rescue-v020`. **Effort**: S.
