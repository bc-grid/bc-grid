...
- `[review: worker1 #187]` **accent-colour-treatment** — extend the theming layer with `--bc-grid-accent` / `-fg` / `-soft` tokens (default: `--primary`). Apply to: selected-row left-edge strip; active-sort column-header treatment; filter input focus border; loading overlay spinner; BcEditGrid action button hover. Pairs with the ERP brand-accent requirement. **Effort**: M.
