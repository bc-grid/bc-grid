...
- `[in-flight: worker1]` **resize-affordance-polish** — v0.2 polish: always-visible, theme-token header resize affordance for resizable columns while preserving the existing resize hit area and pointer behavior. **Branch**: `agent/worker1/resize-affordance-polish`. **Effort**: XS.
