...
- `[in-flight: worker1]` **sort-flip-stability-v040** — stabilize sort FLIP animation: translate-only row motion, stale animation cancellation, and safe skips when row identity/virtualization/size changes make FLIP unsafe. **Branch**: `agent/worker1/sort-flip-stability-v040`. **Effort**: XS.
