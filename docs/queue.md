...
- `[in-flight: worker1]` **layout-persistence-api-v030** — add a consumer-owned JSON-safe layout state contract for restoring column order/width/pinning/visibility, sort/filter/search, grouping, density, sidebar panel, and public pagination state without coupling the grid to localStorage. **Branch**: `agent/worker1/layout-persistence-api-v030`. **Effort**: S.
