...
- `[in-flight: worker1]` **row-focus-selection-chrome-v040** — polish the visual precedence between active cell, focused row, selected row, hover row, and dirty/pending/error cell state markers with token-only shadcn-compatible chrome. **Branch**: `agent/worker1/row-focus-selection-chrome-v040`. **Effort**: XS.
