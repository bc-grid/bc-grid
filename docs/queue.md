...
- `[review: worker1 #245]` **column-resize-affordance-v030** — v0.3 sizing polish: keep column resize handles discoverable before hover without colliding with sorting/menu/filter controls, pinned-column separators, grouped headers, compact density, dark mode, or forced-colors themes. Preserve existing resize hit area and pointer behavior. **Branch**: `agent/worker1/column-resize-affordance-v030`. **Effort**: XS.
...
- `[review: worker4 #273]` **popup-focus-followup** — focus-behavior follow-up to PR #261. Audits popup / menu focus contracts post-roving-focus across context menu, column chooser, filter popup, sidebar tablist + tool panels. Single behavior fix: sidebar tablist switches from `tabIndex={0}` on every tab to roving tabindex (`tabIndex={selected ? 0 : -1}`) per WAI-ARIA Authoring Practices for tabs — Tab now lands once on the active tab and exits the rail; arrow-key roving (already in place) is preserved. Lock-in SSR markup tests for the existing focus contracts (context menu items at tabIndex=-1 with aria-activedescendant on the root; filter popup footer buttons untrapped). Coordination doc gains a slice-9 design note staking out the next implementation slice (refactor `BcGridContextMenu`'s inline keyboard handler onto the shared `useRovingFocus` once #261 lands on main). No new runtime deps; no Escape-handling change (popup-dismiss owns it); no focus traps. **Effort**: S.
