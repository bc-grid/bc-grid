...
- `[review: worker2 #208]` **filter-text-impl-extend** — extend the existing inline text filter with operators (contains / starts-with / ends-with / equals / regex toggle / case-sensitivity toggle). Salvage of closed PR #170 onto post-v0.2 main; coexists with #193 (filter persistence), #200 (clear-state → null), and #203 (showFilterRow). **Effort**: S.
