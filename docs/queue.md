...
- `[in-flight: worker1]` **range-paste-readiness-v040** — v0.4 readiness audit for range copy/paste: paste parsing, valueParser/validate integration, atomic all-or-rollback, disabled/read-only cells, server-grid implications, and focused pure-helper tests. **Branch**: `agent/worker1/range-paste-readiness-v040`. **Effort**: XS.
