...
- `[in-flight: worker3]` **server-paged-contract-hardening-v040** — harden `BcServerGrid rowModel="paged"` contract tests for server total rows, query-driven page reset, stale response ordering, visible-column requests, and no client double-slicing. Branch: `agent/worker3/server-paged-contract-hardening-v040`. **Effort**: S.
