...
| Bundle size — `core` + `virtualizer` + `animations` + `react` | < 150KB gzipped (raised from 100 KiB on 2026-05-03; see §13 decision), plus 10% per-PR drift guard from latest release baseline | rollup output |
...
| 2026-05-03 | **Row-state cascade scoping for nested grids.** Bsncraft v0.5.0-alpha.2 P0 #2 — master `.bc-grid-row:hover` cascaded into nested grid cells via descendant selectors because the detail panel renders inside the master row's DOM. 16 row-state CSS rules in `packages/theming/src/styles.css` gain a `:not(.bc-grid-detail-panel .bc-grid-row)` (row-level) or `:not(.bc-grid-detail-panel .bc-grid-cell)` / `:not(.bc-grid-detail-panel .bc-grid-cell-pinned-{left,right})` (cell-level / pinned-cell-level) cascade-scoping guard. Universal browser support (Selectors L4, Chrome 88+ / Firefox 84+ / Safari 14.1+); +2 specificity uniformly applied so cascade ordering is preserved. The CSS `@scope` alternative (cleaner but Chrome 118+ / Safari 17.4+ / Firefox 128+) is documented as a v0.7 follow-up once browser baselines catch up. Closes `row-state-cascade-scoping-rfc.md`. | JohnC + Claude coordinator |
